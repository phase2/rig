...
func (cmd *Doctor) Run(c *cli.Context) error {
	...
	if !util.SupportsNativeDocker() {
		...
		if cmd.machine.Exists() {
			if _, isset := os.LookupEnv("DOCKER_MACHINE_NAME"); !isset {
				...
				cmd.out.Error(fmt.Sprintf("Your environment configuration specifies a different machine. Please re-run as 'rig --name=\"%s\" doctor'.", cmd.machine.Name))
				return cmd.Error("Could not complete.", "DOCTOR-FATAL", 1)
			} else {
				cmd.out.Info(fmt.Sprintf("Docker Machine (%s) name matches your environment configuration.", cmd.machine.Name))
			}
			if output, err := exec.Command("docker-machine", "url", cmd.machine.Name).Output(); err == nil {
				...
				cmd.out.Info(fmt.Sprintf("Docker Machine (%s) URL (%s) matches your environment configuration.", cmd.machine.Name, hostURL))
			}
		} else {
			cmd.out.Error(fmt.Sprintf("No machine named '%s' exists. Did you run 'rig start --name=\"%s\"'?", cmd.machine.Name, cmd.machine.Name))
			return cmd.Error("Could not complete.", "DOCTOR-FATAL", 1)
		}
	}
	...
	if !util.SupportsNativeDocker() {
		...
		if !cmd.machine.IsRunning() {
			cmd.out.Error(fmt.Sprintf("Docker Machine '%s' is not running. You may need to run 'rig start'.", cmd.machine.Name))
			return cmd.Error(fmt.Sprintf("Machine '%s' is not running. ", cmd.machine.Name), "DOCTOR-FATAL", 1)
		}
		cmd.out.Info(fmt.Sprintf("Docker Machine (%s) is running", cmd.machine.Name))
	} else {
	...
	if err != nil {
		cmd.out.Error(fmt.Sprintln("Could not determine Docker Machine Docker versions: ", err))
	} else if clientAPIVersion.Equal(serverAPIVersion) {
		cmd.out.Info(fmt.Sprintf("Docker Client (%s) and Server (%s) have equal API Versions", clientAPIVersion, serverAPIVersion))
	} else if apiConstraint.Check(clientAPIVersion) {
		cmd.out.Info(fmt.Sprintf("Docker Client (%s) has Server compatible API version (%s). Server current (%s), Server min compat (%s)", clientAPIVersion, constraintString, serverAPIVersion, serverMinAPIVersion))
	} else {
		cmd.out.Error(fmt.Sprintf("Docker Client (%s) is incompatible with Server. Server current (%s), Server min compat (%s). Use `rig upgrade` to fix this.", clientAPIVersion, serverAPIVersion, serverMinAPIVersion))
	}
	...
	if records, err := dnsRecords.LoadRecords(); err == nil {
		...
		for _, record := range records {
			if record["Name"] == "dnsdock" {
				resolved = true
				cmd.out.Info(fmt.Sprintf("DNS and routing services are working. DNSDock resolves to %s", record["IPs"]))
				break
			}
		}
		...
		cmd.out.Error(fmt.Sprintf("Unable to verify DNS services and routing are working: %s", err.Error()))
	}
	...
	if !util.SupportsNativeDocker() {
		...
		if err == nil {
			...
			if i, e := strconv.Atoi(dataUsage); e == nil {
				if i >= 85 && i < 95 {
					cmd.out.Warning(fmt.Sprintf("Data volume (/data) is %d%% used. Please free up space soon.", i))
				} else if i >= 95 {
					cmd.out.Error(fmt.Sprintf("Data volume (/data) is %d%% used. Please free up space. Try 'docker system prune' or removing old projects / databases from /data.", i))
				} else {
					cmd.out.Info(fmt.Sprintf("Data volume (/data) is %d%% used.", i))
				}
			} else {
				cmd.out.Warning(fmt.Sprintf("Unable to determine usage level of /data volume. Failed to parse '%s'", dataUsage))
			}
		} else {
			cmd.out.Warning(fmt.Sprintf("Unable to determine usage level of /data volume. Failed to execute 'df': %v", err))
		}
	}
	...
	if !util.SupportsNativeDocker() {
		...
		if err == nil {
			...
			if i, e := strconv.Atoi(userUsage); e == nil {
				if i >= 85 && i < 95 {
					cmd.out.Warning(fmt.Sprintf("Root drive (/Users) is %d%% used. Please free up space soon.", i))
				} else if i >= 95 {
					cmd.out.Error(fmt.Sprintf("Root drive (/Users) is %d%% used. Please free up space.", i))
				} else {
					cmd.out.Info(fmt.Sprintf("Root drive (/Users) is %d%% used.", i))
				}
			} else {
				cmd.out.Warning(fmt.Sprintf("Unable to determine usage level of root drive (/Users). Failed to parse '%s'", userUsage))
			}
		} else {
			cmd.out.Warning(fmt.Sprintf("Unable to determine usage level of root drive (/Users). Failed to execute 'df': %v", err))
		}
	}
